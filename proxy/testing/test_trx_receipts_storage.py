...
class TestTrxReceiptsStorage(TestCase):
    ...
    def test_data_consistency(self):
        ...
        """
        self.trx_receipts_storage.clear()
        self.assertEqual(self.trx_receipts_storage.size(), 0)
        self.assertEqual(self.trx_receipts_storage.max_known_trx(), (0, None))
        ...
        for _ in range(0, num_items):
            ...
            self.trx_receipts_storage.add_trx(slot, signature, trx)
            expected_items.append((slot, signature, trx))

        self.assertEqual(self.trx_receipts_storage.max_known_trx()[0], max_slot)
        self.assertEqual(self.trx_receipts_storage.size(), num_items)
        for item in expected_items:
            self.assertTrue(self.trx_receipts_storage.contains(item[0], item[1]))
    ...
    def test_query(self):
        ...
        retrieved_trxs = [item for item in self.trx_receipts_storage.get_trxs(start_slot, False)]
        ...
        retrieved_trxs = [item for item in self.trx_receipts_storage.get_trxs(start_slot, True)]
