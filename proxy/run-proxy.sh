...
if [ "$CONFIG" == "ci" ]; then
  ...
  [[ -z "$NEW_USER_AIRDROP_AMOUNT"  ]] && export NEW_USER_AIRDROP_AMOUNT=10000000
...
if [ -n "$NEW_USER_AIRDROP_AMOUNT" ]; then
  ...
  /spl/bin/spl-token mint $ETH_TOKEN_MINT $NEW_USER_AIRDROP_AMOUNT"00" --owner /spl/bin/test_token_owner -- $TOKEN_ACCOUNT
