...
read -r -d '' MODIFIED_CHUNK_RESPONSE_PLUGIN << EOM
modify
chunk
response
plugin
...
echo "[Test Modified Chunk Response Plugin Proxy]"
CMD="curl -v -x $PROXY_URL https://httpbin.org/stream/5"
RESPONSE=$($CMD 2> /dev/null)
verify_response "$RESPONSE" "$MODIFIED_CHUNK_RESPONSE_PLUGIN"
VERIFIED1=$?
