...
class Proxy(multiprocessing.Process):
    ...
    def __init__(self, client, delay=0.0):
    ...
    def _process_rlist(self, r):
        if self.client.conn in r:
            ...
            try:
                # add a delay if you want to emulate bad network conditions
                if (self.delay != 0.0):
                    logger.debug('waiting for %f seconds', self.delay)
                    sleep(self.delay)
...
class TCP(object):
    ...
    def __init__(self, hostname='127.0.0.1', port=8899, backlog=100, delay=0.0):
    ...
    def run(self):
        try:
            logger.info('Starting server on port %d with delay of %f',self.port, self.delay)
...
class HTTP(TCP):
    ...
    def handle(self, client):
        proc = Proxy(client, delay=self.delay)
        proc.daemon = True
        proc.start()
...
def main():
    ...
    parser.add_argument('--delay', default=0.0, help="Delay in floating point seconds")
    ...
    try:
        proxy = HTTP(hostname, port, delay=delay)
        proxy.run()
    except KeyboardInterrupt:
        pass
